...
public class GroupRepository : IGroupRepository
{
    ...
    {
        using var _context = await _contextFactory.CreateDbContextAsync();
        var groupModel = await _context.Groups.AsNoTracking().AsSplitQuery().Include(g => g.Participants).ThenInclude(gp => gp.User).Include(g => g.Assignments).ThenInclude(a => a.Participants).ThenInclude(ap => ap.User).FirstOrDefaultAsync(g => g.GroupId == id, cancellationToken);
